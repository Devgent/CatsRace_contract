...
contract CatsRace is Context, IBEP20, Ownable {
    ...
    string private _name     = "CatsRace";
    string private _symbol   = "CatsRace";
    ...
    function calculateSystemFee(uint256 amount) private view returns (uint256) {
        //uint256 marketingFeeInPercent = _marketingFee.div(_liquidityFee);
        return amount.mul(_systemFee).div(_liquidityFee);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        */
        ...
        // take marketing fee
        uint256 systemFee   	= calculateSystemFee(newBalance);
        uint256 fee   	= systemFee.div(2);
        uint256 bnbForLiquidity = newBalance.sub(systemFee);

        if (systemFee > 0) {
            payable(_marketingWallet).transfer(fee);
            emit MarketingFeeSent(_marketingWallet, fee);

            payable(_devWallet).transfer(fee);
            emit DevFeeSent(_devWallet, fee);
        }
